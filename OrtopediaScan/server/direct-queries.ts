...
export async function createMedicalOrderDirect(orderData: any) {
  ...
  try {
    ...
    const result = await pool.query(query, [
      ...
      orderData.procedureCbhpmId || null,
      orderData.procedureCbhpmQuantity || 1,
      orderData.secondaryProcedureIds?.length ? orderData.secondaryProcedureIds : emptyArray,
      orderData.secondaryProcedureQuantities?.length ? orderData.secondaryProcedureQuantities : emptyArray,
      orderData.secondaryProcedureLateralities?.length ? orderData.secondaryProcedureLateralities : emptyArray,
