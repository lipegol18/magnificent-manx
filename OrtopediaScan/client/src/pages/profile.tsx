...
const Profile = () => {
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Invalidar cache para atualizar dados do usuário em todas as páginas
  ...
  // Função para lidar com o upload da assinatura
  ...
  const handleSignatureCropConfirm = async () => {
    ...
    try {
      console.log('🎯 Iniciando crop da assinatura...');
      ...
      await handleSignatureUpload(croppedFile);
    } catch (error) {
      console.error('❌ Erro ao processar crop da assinatura:', error);
      toast({
        title: "Erro",
        description: "Erro ao processar a imagem: " + (error as Error).message,
        variant: "destructive",
      });
    }
  };
  ...
  const handleSignatureUpload = async (fileToUpload?: File) => {
    ...
    try {
      ...
      // Invalidar cache para forçar nova busca dos dados do usuário
      queryClient.invalidateQueries({ queryKey: ["/api/user"] });
  ...
  return (
    <div className="container mx-auto py-6">
      <h1 className="text-3xl font-bold mb-6 text-foreground">{t('nav.profile')}</h1>
